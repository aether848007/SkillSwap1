...
public interface SkillRepository extends JpaRepository<Skill, UUID> {
    @Query("SELECT s FROM Skill s JOIN FETCH s.skillProfile sp JOIN FETCH sp.user " +
           "WHERE s.isOffered = true AND s.isActive = true AND " +
           "(LOWER(s.title) LIKE LOWER(CONCAT('%', :query, '%')) OR " +
           "LOWER(s.description) LIKE LOWER(CONCAT('%', :query, '%')))")
    List<Skill> searchSkills(@Param("query") String query);

    @Query("SELECT s FROM Skill s JOIN FETCH s.skillProfile sp JOIN FETCH sp.user " +
           "WHERE s.isOffered = true AND s.isActive = true AND s.category = :category")
    List<Skill> findByActiveCategory(@Param("category") SkillCategory category);

    @Query("SELECT s FROM Skill s JOIN FETCH s.skillProfile sp JOIN FETCH sp.user " +
           "WHERE s.isOffered = true AND s.isActive = true")
